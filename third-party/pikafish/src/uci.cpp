...
void UCIEngine::loop() {
    ...
    do
    {
        if (cli.argc == 1
            ...
            cmd = "quit";
    } while (execute(cmd) && cli.argc <= 1);  // Command-line arguments are one-shot
}

bool UCIEngine::execute(const std::string& command) {
    currentCmd = command;
    ...
    else if (token == "export_net")
    {
        ...
        sync_cout << "Unknown command: '" << command << "'. Type help for more information."
                  << sync_endl;

    return token != "quit";
}

bool UCIEngine::validate_move(const std::string& fen, const std::string& move) {
    engine.stop();
    engine.wait_for_search_finished();
    const auto error = engine.set_position(fen, {move});
    if (error)
        engine.set_position(fen, {});
    return !error;
}

u64 UCIEngine::legal_move_count(const std::string& fen) {
    engine.stop();
    engine.wait_for_search_finished();
    const auto result = engine.perft(fen, 1);
    return std::holds_alternative<u64>(result) ? std::get<u64>(result) : 0;
}
